...
pub fn structurize(cfg: &ControlFlowGraph, fir: &FunctionIR) -> StructuredBlock {
    ...
    let doms = cfg_analysis_old::compute_dominators(cfg, entry);
    let back_edges = cfg_analysis_old::find_back_edges(cfg, &doms);
    // 构建回边映射：to -> Vec<from>
...
fn structurize_postorder(
    ...
    if let Some(back_froms) = back_edge_map.get(&block_id) {
        ...
        for &tail in back_froms {
            let body = cfg_analysis_old::collect_natural_loop(cfg, block_id, tail);
